...
def detect_click():
    global total_amount, target_reached, net, cap, classes, target_amount
    while True:
        # Check for keypress
        ret, frame = cap.read()
        key = cv2.waitKey(1)
    ...
    detect_button = ttk.Button(window, text="Detect", command=detect_click)
    detect_button.pack(side=tk.LEFT, padx=(10, 10))

    # Create the "Quit" button
    quit_button = ttk.Button(window, text="Quit", command=quit_click)
    quit_button.pack(side=tk.LEFT, padx=(10, 10))
